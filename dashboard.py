...
# ========== PATH FILE - SESUAIKAN DENGAN NAMA FILE DI GITHUB ==========
...
def load_and_process_data(path_2024, path_2025):
    ...
    try:
        ...
        # Bersihkan tipe laporan - JANGAN ubah ke lowercase!
        if 'TIPE LAPORAN' in df.columns:
            df['TIPE LAPORAN'] = df['TIPE LAPORAN'].astype(str).str.strip()
            df['TIPE LAPORAN_lower'] = df['TIPE LAPORAN'].str.lower()
        else:
            df['TIPE LAPORAN'] = 'unknown'
            df['TIPE LAPORAN_lower'] = 'unknown'
        ...
        if all(col in df.columns for col in ['LATITUDE', 'LONGITUDE', 'DESKRIPSI']):
            # Konversi latitude longitude ke numeric
            df['LATITUDE'] = pd.to_numeric(df['LATITUDE'], errors='coerce')
            df['LONGITUDE'] = pd.to_numeric(df['LONGITUDE'], errors='coerce')
            
            df['is_ghost_auto'] = (
                (df['duration_seconds'].fillna(999999) == 0) &
                (df['DESKRIPSI'].fillna('').astype(str).str.len() < 5) &
        ...
        # === DETEKSI PRANK CALL ===
        df['prank_call'] = df['TIPE LAPORAN_lower'] == 'prank'
        ...
        return None, f"❌ File tidak ditemukan: {e}\n\nPastikan file Excel ada di folder yang sama dengan dashboard.py"
...
def main():
    ...
    if error:
        st.error(error)
        st.info("💡 **Tips:**\n- Pastikan nama file Excel PERSIS SAMA dengan yang di code\n- Cek apakah file ada di folder yang benar")
        return
    ...
    with tab1:
        ...
        with col1:
            ...
            with col1:
                ...
                # Filter ghost call first, then groupby
                ghost_df = df_filtered[df_filtered['ghost_call'] == True]
                if len(ghost_df) > 0:
                    ghost_by_agent = ghost_df.groupby('AGENT L1').size().sort_values(ascending=False).head(10)
                    ...
                    if len(ghost_by_agent) > 0:
                        ...
                        # Tabel detail
                        ghost_table = ghost_by_agent.reset_index()
                        ghost_table.columns = ['Agent', 'Jumlah Ghost Call']
            ...
            with col2:
                ...
                # Filter prank call first, then groupby
                prank_df = df_filtered[df_filtered['prank_call'] == True]
                if len(prank_df) > 0:
                    prank_by_agent = prank_df.groupby('AGENT L1').size().sort_values(ascending=False).head(10)
                    ...
                    if len(prank_by_agent) > 0:
                        ...
                        # Tabel detail
                        prank_table = prank_by_agent.reset_index()
                        prank_table.columns = ['Agent', 'Jumlah Prank Call']
